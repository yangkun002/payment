/*
 * 文件名：DateUtil.java
 * 版权：Copyright 2018 汪涛. All Rights Reserved.
 * 描述： 日期时间计算工具类
 * 创建人：汪涛
 * 创建时间：2015-07-14
 */
...
public class WzDateUtil {
    ...
    public static String dt2Str(Date dt, String fmt) {
        String usedFmt = WzStringUtil.isBlank(fmt) ? "yyyyMMddHHmmss" : fmt;
    ...
    public static Date str2Dt(String dtStr, String fmt) {
        if (WzStringUtil.isBlank(dtStr) || WzStringUtil.isBlank(fmt)) {
            return null;
        }
    ...
    public static String getTimeByHmsS(String date, Integer hmss) {
        ...
        if (WzStringUtil.isNotBlank(date) && WzStringUtil.isNumeric(date) && 8 == date.length()) {
            dateTimeStr = date;
        } else {
