 ...
 */
public class WzEncryptUtil {
    /** 日志对象 */
    private static final Logger logger = LoggerFactory.getLogger(WzEncryptUtil.class);
    ...
    public static final String stringToBase64(String str, String encoding) {
        ...
        if (WzStringUtil.isBlank(encoding)) {
    ...
    public static String decryptHexString(String key, String charset) {
        try {
            if (WzStringUtil.isBlank(charset)) {
    ...
    public static final String encryptSHA256String(String value) {
        ...
        if (null != result) {
            return WzStringUtil.parse16(result);
        }
        return "";
    }
    ...
    public static String getMD5(String str, boolean isUpperCase) {
        ...
        try {
            ...
            md.update(buffer);
            byte[] md5StrArr = md.digest();
            // 返回MD5加密后16进制字符串
            String md5Str = WzStringUtil.parse16(md5StrArr);
            if (WzStringUtil.isEmpty(md5Str)) {
                return "";
            } else {
                if (isUpperCase) {
